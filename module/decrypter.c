...
static struct class*  deccharClass  = NULL; ///< The device-driver class struct pointer
static struct device* deccharDevice = NULL; ///< The device-driver device struct pointer
...
static int __init decrypt_init(void){
   ...
   deccharClass = class_create(THIS_MODULE, CLASS_NAME);
   if (IS_ERR(deccharClass)){                // Check for error and clean up if there is
      unregister_chrdev(majorNumber, DEVICE_NAME);
      printk(KERN_ALERT "Fallo al registrar clase del dispositivo\n");
      return PTR_ERR(deccharClass);          // Correct way to return an error on a pointer
   }
 
   // Registro del driver del dispositivo
   deccharDevice = device_create(deccharClass, NULL, MKDEV(majorNumber, 0), NULL, DEVICE_NAME);
   if (IS_ERR(deccharDevice)){               // Clean up si hay algun error
      class_destroy(deccharClass);           // Repeated code but the alternative is goto statements
      unregister_chrdev(majorNumber, DEVICE_NAME);
      printk(KERN_ALERT "Fallo al crear dispositivo\n");
      return PTR_ERR(deccharDevice);
   }
...
static void __exit decrypt_exit(void){
   ...
   device_destroy(deccharClass, MKDEV(majorNumber, 0));     // remuevo el dispositivo
   class_unregister(deccharClass);                         
   class_destroy(deccharClass);                             
   unregister_chrdev(majorNumber, DEVICE_NAME);
